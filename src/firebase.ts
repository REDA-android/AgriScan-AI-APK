...
export const firebaseConfig: any = hasValidConfig ? configData : {
...
export const uploadImage = async (file: Blob, filePath: string, onProgress?: (progress: number) => void) => {
  ...
  if (file.size > MAX_FILE_SIZE) {
    throw new Error(`La taille du fichier (${(file.size / (1024 * 1024)).toFixed(1)} MB) dépasse la limite autorisée de 4 MB. Veuillez utiliser une image plus petite pour garantir une synchronisation rapide.`);
  }
...
export const signInWithGoogle = async (lang?: string) => {
  if (isSigningIn) {
  ...
  try {
    ...
    // Connexion Google standard via popup Firebase
    try {
      ...
      if (popupError.code === 'auth/popup-blocked') {
        window.dispatchEvent(new CustomEvent('app-notify', { 
          detail: { message: "La fenêtre de connexion a été bloquée. Veuillez autoriser les popups pour vous connecter.", type: 'error' } 
        }));
        throw popupError;
      }
      ...
      if (popupError.code === 'auth/unauthorized-domain') {
        ...
        console.error(`Ce domaine (${currentDomain}) n'est pas autorisé dans votre console Firebase.`);
        throw popupError;
      }
    ...
    if (error.code === 'auth/popup-closed-by-user' || error.code === 'auth/cancelled-popup-request') {
      ...
      const msg = `Erreur d'authentification (${error.code}) : ${error.message}.`;
      console.error(msg);
...
export const checkRedirectResult = async () => {
  ...
  redirectResultPromise = (async () => {
    try {
      ...
      // Re-throw critical authentication errors so the application can catch and display them
      if (
